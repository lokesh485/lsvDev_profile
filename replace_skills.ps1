...
# Find <!-- EXPERIENCE --> which comes after skills
$expIdx = $content.IndexOf('<!-- EXPERIENCE -->')
if ($expIdx -lt 0) { Write-Host "END MARKER NOT FOUND"; exit 1 }

# Replace from <!-- SKILLS --> up to (but not including) <!-- EXPERIENCE -->
$before = $content.Substring(0, $startIdx)
$after  = $content.Substring($expIdx)

$newContent = $before + $newSkills + "`r`n`r`n" + $after

[System.IO.File]::WriteAllText((Resolve-Path $file).Path, $newContent, [System.Text.Encoding]::UTF8)
Write-Host "Skills section replaced successfully. Total length: $($newContent.Length)"
